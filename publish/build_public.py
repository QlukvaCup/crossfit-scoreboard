...
def _serialize_workouts(settings: Dict[str, Any]) -> Dict[str, Any]:
    structure = settings.get("workout_structure") if isinstance(settings.get("workout_structure"), list) else []
    raw_workouts = settings.get("workouts") if isinstance(settings.get("workouts"), dict) else {}
    payload: Dict[str, Any] = {"structure": structure, "divisions": {}}
    for div in DIVISIONS:
        div_id = div["id"]
        div_rows: List[Dict[str, Any]] = []
        current = raw_workouts.get(div_id) if isinstance(raw_workouts.get(div_id), dict) else {}
        for row in structure:
            ...
            for part in parts or [""]:
                ...
                code = f"{base}{suffix}" if base else ""
                if not code:
                    continue
                ...
                div_rows.append({
                    ...
                    "time_cap": str(item.get("time_cap") or "").strip(),
                    "description": str(item.get("description") or "").strip(),
                    "base": base,
                })
        payload["divisions"][div_id] = {"title": div["title"], "rows": div_rows}
    return payload
...
def build_public_payload() -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        ...
        "workouts": _serialize_workouts(settings),
    }
